...
const formatPriceToString = (
  ...
  const integerPartInWords = toWords(integerPart, { lang: language });
  const fractionalPartInWords =
    fractionalPart > 0 ? toWords(fractionalPart, { lang: language }) : null;
  ...
  let result =
    integerPartInWords.charAt(0).toUpperCase() + integerPartInWords.slice(1);
  ...
  if (fractionalPartInWords) {
    if (afterDecimal) {
      result += ` et ${fractionalPartInWords} ${afterDecimal}`;
    } else {
      result += ` virgule ${fractionalPartInWords}`;
    }
  }
